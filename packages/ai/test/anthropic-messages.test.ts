...
describe("streamAnthropicMessages", () => {
	...
	it("retries a 429 then streams normally, reporting exactly one onRetry", async () => {
		const okSse = sseFromEvents([messageStart(), ...messageStopSequence("end_turn")]);
		let calls = 0;
		const fetchMock: typeof fetch = async () => {
			calls += 1;
			if (calls === 1) {
	...
	it("ends with an error naming the last status after retries are exhausted", async () => {
		...
		expect(final.stopReason).toBe("error");
		expect(final.errorMessage).toMatch(/^Anthropic Messages HTTP 503/u);
	...
	it("turns a stalled body into a `Stream idle for` error", async () => {
		...
		const hanging = new ReadableStream<Uint8Array>({
			start(controller) {
				controller.enqueue(
					encoder.encode(`event: message_start\ndata: ${JSON.stringify(messageStart().data)}\n\n`),
				);
