...
describe("streamOpenAICompletions", () => {
	...
	it("retries a 429 then streams normally, reporting exactly one onRetry", async () => {
		const okSse = sseFromChunks([{ choices: [{ index: 0, delta: {}, finish_reason: "stop" }] }]);
		let calls = 0;
		const fetchMock: typeof fetch = async () => {
			calls += 1;
			if (calls === 1) {
	...
	it("ends with an error naming the last status after retries are exhausted", async () => {
		...
		expect(events.at(-1)?.type).toBe("error");
		expect(final.stopReason).toBe("error");
		expect(final.errorMessage).toMatch(/^OpenAI Completions HTTP 503/u);
	...
	it("turns a stalled body into a `Stream idle for` error", async () => {
		...
		const hanging = new ReadableStream<Uint8Array>({
			start(controller) {
				controller.enqueue(
					encoder.encode(`data: ${JSON.stringify({ choices: [{ index: 0, delta: { content: "hi" } }] })}\n\n`),
				);
