...
import { loadSkillIndex, loadSkills } from "../src/skills.ts";
...
describe("skills", () => {
	it("loads SKILL.md from cwd .pillow/skills", async () => {
		...
		expect(skills.some((skill) => skill.metadata.name === "demo")).toBe(true);
		expect(skills[0]?.body).toBeUndefined();
		expect((await loadSkillIndex(cwd)).byName.has("demo")).toBe(true);
	});
});
